...
experis::DungeonAscii::DungeonAscii(bool a_up,
								  bool a_right,
								  bool a_down,
								  bool a_left,
								  bool a_isDragon,
...
void DrawHorizontalLine(experis::Grid& a_canvas, int a_xLeft, int a_xRight, int a_y)
{
	for (int i = a_xLeft; i <= a_xRight; ++i)
	{
		a_canvas(i, a_y) = experis::HORIZONTAL_WALL_SYMBOL;
	}
}

void DrawVerticalLine(experis::Grid& a_canvas, int a_yUp, int a_yDown, int a_x)
{
	for (size_t i = a_yUp; i < a_yDown; ++i)
	{
		a_canvas(a_x, i) = experis::VERTICAL_WALL_SYMBOL;
	}
}


void experis::DungeonAscii::Draw(Grid& a_canvas, Direction a_direction, int a_baseX, int a_baseY) const
{
	// room corners
	int xLeft = 2;
...
void experis::DungeonAscii::DrawPlayer(Grid & a_canvas, int a_baseX, int a_baseY, Direction a_direction) const
{
	switch (a_direction)
	{
		case experis::Direction::NORTH:
		{
		...
		case experis::Direction::WEST:
		{
			a_canvas(a_baseX, a_baseY) = '\\';
			a_canvas(a_baseX, a_baseY + 1) = '/';
			break;
		}
		case experis::Direction::SOUTH:
		{
			a_canvas(a_baseX, a_baseY + 1) = '\\';
			a_canvas(a_baseX + 1, a_baseY + 1) = '/';
			a_canvas(a_baseX - 1, a_baseY) = '\\';
